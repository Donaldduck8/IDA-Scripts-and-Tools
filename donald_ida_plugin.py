...
class FLOSSActionHandler(ida_kernwin.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        original_func = floss_ida_script.apply_stack_strings

        def hook(stack_strings, tight_strings, lvar_cmt = True, cmt = True) -> None:
            original_func(stack_strings, tight_strings, lvar_cmt, cmt)
        ...
        floss_ida_script.apply_stack_strings = hook
...
class DonaldPlugin(ida_idaapi.plugin_t):
    ...
    def init(self):
        ...
        if inf.filetype in [ida_ida.f_EXE_old, ida_ida.f_EXE, ida_ida.f_PE, ida_ida.f_W32RUN, ida_ida.f_WIN]:
            # Create an action
            action = ida_kernwin.action_desc_t(
                "donald_plugin:floss",
                "Floss",
